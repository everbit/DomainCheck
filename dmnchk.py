...
def process_domain(domain, include_subdomains=False, should_check_subdomain_status=False, custom_user_agent=None, email_security_checks=False, interactive=False, original_url=None, skip_checks=False):
    logging.info(f"Processing domain: {domain}")

    sanitized_domain = None

    try:
        # Use the provided original_url if available for the status check, otherwise default to the sanitized domain
...
def interactive_mode():
    ...
    while True:
        ...
        if proceed_choice == 'yes':
            process_domain(domain, include_subdomains, should_check_subdomain_status, custom_user_agent, email_security_checks=email_security_checks, interactive=True)
            break
...
def main():
    ...
    if args.interactive:
        ...
        if args.domain:
            process_domain(args.domain, args.subdomains, args.check_subdomain_status, args.user_agent, email_security_checks=args.email_security_checks)
